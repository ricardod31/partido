...
| **Compartimento** | Um organismo visto como unidade de isolamento: cada organismo é um compartimento criptográfico (grupo MLS) **e** de navegação — nenhuma tela mistura dois, e a troca entre eles é um gesto deliberado (UX1). | [doc 07](docs/07-paginas/00-design-system.md), [doc 03](docs/03-arquitetura-criptografica.md) |
...
| **Fail-closed** | Comportamento do cliente de **recusar** operações sensíveis (sobretudo depositar voto) quando não há canal anônimo (Tor) confirmado, em vez de cair para clearnet em silêncio. | [doc 06](docs/06-modelo-de-ameacas.md), [doc 03](docs/03-arquitetura-criptografica.md) |
...
| **Handle (por-organismo)** | Identificador de exibição de uma pessoa **local a um organismo** e não-vinculável entre organismos (em vez de um `user_id` global), para que a autoria e a filiação não reconstruam o grafo por pessoa. | [ADR-0008](docs/decisoes/adr-0008-mls-e-credenciais-anonimas.md), [doc 07](docs/07-paginas/00-design-system.md) |
