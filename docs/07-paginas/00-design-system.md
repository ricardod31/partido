...
### 11.1 Tabela única de honestidade (canônica) [conceitual]

**Fonte única** das strings de honestidade recorrentes: protótipo e build citam por chave; texto que divirja desta tabela é **defeito**, não variação editorial. A política da §11 aplica-se a cada linha: a **íntegra** aparece na primeira exposição (e sempre, para recém-chegada — P-AJU-06); o **resumo** de 1 linha aparece depois, expansível de volta à íntegra; avisos acoplados a bloqueio real são sempre íntegros. A coluna final é o guarda-corpo: o que nenhuma reescrita futura pode passar a prometer.

| Chave | Onde | Íntegra (1ª exposição) | Resumo (veterana) | Nunca pode prometer |
|---|---|---|---|---|
| `voto.coacao` | P-DEL-06/07, cerimônia de voto | O voto é secreto perante o sistema. Mas ele **não** te protege se alguém te obriga a mostrar como votou. | Secreto — mas não protege contra quem te obriga a mostrar. | resistência a coação |
| `voto.verificacao` | P-DEL-08 (apuração) | Você confere que não houve mais senhas que eleitores, nem mais cédulas que senhas. O que você **não** confere, nesta versão, é que a **sua** cédula foi contada. | Confere os totais; ainda não o seu voto individual. | verificabilidade individual (antes de DEP-09 resolvido) |
| `voto.sala-pequena` | P-DEL-02/06 | Numa sala pequena, o placar pode entregar o voto: num 3 a 0, todos sabem como todos votaram — aritmética, não falha. | Sala pequena: o placar pode entregar o voto. | sigilo que a aritmética nega |
| `nominal.registro` | P-DEL-05, P-MAN-09 | Voto **de rosto**: fica registrado quem votou o quê. Não abrir no prazo conta como **abstenção registrada** — segurar o voto não dá vantagem. | De rosto: registrado; não abrir = abstenção registrada. | apagar o registro depois |
| `mural.metadados` | P-ORG-02 (TrustChip → folha) | Protegido · o servidor vê: **para qual grupo, quando, tamanho** — não o conteúdo, não qual membro escreveu (vê "um dos N"). | **"Protegido · o servidor vê"** (variante curta de linha fixa, emenda do ciclo 7: 1ª exposição e novata veem a íntegra; a folha carrega a íntegra sempre) | "o servidor não vê nada" |
| `fechadura.need-to-know` | mural, P-ORG-04/09 | Quem **entra** não lê o que veio antes; quem **sai** não lê o que vem depois. A porta fecha nos dois sentidos. **Isso é desenho, não falha.** | A porta fecha nos dois sentidos. | acesso retroativo por conveniência |
| `chave-mestra.perda` | P-ON-04, P-SEG-01, L2 | Perdeu estas palavras **e** o aparelho? A conta se perde — ninguém no servidor pode devolvê-la. É desenho, não descuido. | Sem papel e sem aparelho, a conta se perde — por desenho. | recuperação pelo servidor |
| `recuperacao.historico` | P-ON-09 | A recuperação refaz suas chaves e suas salas te reconhecem. O histórico do aparelho antigo **não volta** — e o de antes da sua entrada em cada sala segue fechado. Ninguém é avisado de uma recuperação: quem tem as palavras é você, para todos os efeitos. | Volta a identidade; histórico local, não. | restauração de histórico; alerta automático de restauração |
| `emparelhar.presencial` | P-ON-10 | Só entre aparelhos **seus**, os dois na sua mão, no mesmo lugar. Quem fotografa ou filma este código **vira você**. | Presencial, entre aparelhos seus — o código é a conta. | pareamento remoto seguro |
| `vale.portador` | P-FIN-02/05 | O vale **vale como dinheiro**: quem estiver com o código pode usá-lo. Guarde-o até resgatar. | Vale como dinheiro — guarde até resgatar. | rastrear/estornar um vale |
| `pix.identificado` | P-FIN-03 (regime partido) | Pagamento **identificado**: o banco — e a prestação de contas pública — ligam **seu nome** ao valor. Exigência da lei para partido registrado, não escolha do app. | Identificado: banco e prestação pública veem seu nome — lei. | anonimato sob regime de partido |
| `infiltrado` | ExposurePanel, L4 | Um **infiltrado** — colega de verdade da sua sala — lê o que a sala lê. Nenhuma criptografia ajuda contra isso; a defesa é a compartimentação e o critério de quem entra. | Colega de verdade lê o que a sala lê. | "a criptografia resolve infiltração" |
| `pseudonimo.rede` | P-ON-03 | Escolha um apelido sem ligação com seu nome, trabalho ou redes. E: no mesmo aparelho, seus apelidos podem ser **ligados por rede e horário** — apelido não é anonimato de rede; para isso existe a rede protegida. | Apelido não é anonimato de rede. | não-vinculabilidade sem canal anônimo |
| `convite.nominal` | P-ON-02, P-ORG-10 (enquanto DEP-03 aberto) | Por ora, o servidor vê **qual secretário assinou** este convite. | (já é curto) | convite cego antes de existir |
| `estrutura.servidor` | P-NAV-02, P-ORG-07, ExposurePanel | A **estrutura** — que salas existem e como se ligam — fica no servidor: sem ela, ele não entrega as mensagens. É o que uma apreensão expõe; o **conteúdo**, não. | A estrutura fica no servidor; o conteúdo, não. | negar o grafo exposto (A3) |
| `agregador.captura` | P-NAV-01/02, P-SEG-01 | Esta tela junta suas salas — **só no seu aparelho**. Uma foto dela entrega seu mapa inteiro: é a tela mais reveladora do app. | Só no seu aparelho — uma captura entrega seu mapa. | que agregar seja sem custo |
| `ensaio.nada-vale` | P-AJU-04 (selo permanente) | **ENSAIO — nada disto vale de verdade.** Nenhuma cédula real, nada sai do aparelho, nada conta. | (o selo é permanente e íntegro) | qualquer efeito real do ensaio |
| `aprendizado.sem-protecao-nova` | P-AJU-02 | Ler aqui **não** te dá proteção nova: as proteções são iguais para quem nunca leu. O que muda é você decidir sabendo. | Ler não dá proteção nova. | leitura como segurança |

**Rótulos canônicos do TrustChip** (§2): `Protegido` (verde, cadeado), `Exposto` (âmbar, olho), `Não protege` (vermelho), `Anônimo` (violeta) — sempre **condicionados ao estado real do build** (regra do §2): um chip nunca anuncia proteção que a versão corrente não entrega.
...
- **Alta fidelidade — estado após o ciclo 5 do protótipo.** A primeira versão de alta fidelidade **fixou** (valores vivos em [/prototipo/index.html](../../prototipo/index.html), a extrair para tokens formais; sujeitos a polimento, não a redesenho): **estados interativos** (hover = superfície +1 nível, pressed = translate 1px, disabled = 45% sem pointer) e **focus ring** (2 px `--accent`, offset 2 px, `:focus-visible`); **anatomia v1 dos componentes-chave** (TrustChip com fundo tintado por papel, DeliberationStepper com passo corrente em caixa da espinha + halo, FailClosedBlocker de tela cheia, Ceremony com topo/rodapé fixos e corpo rolável, PracticeFrame com contorno tracejado + faixa listrada, ResolutionAta serifada, cartão do Panorama com borda-espinha); **espinha de compartimento** (4 px, borda esquerda da moldura, transição 240 ms `cubic-bezier(.2,.7,.3,1)`, tinta de 9% no cabeçalho do compartimento); **escala de elevação/z** (espinha 40 &lt; cerimônia 45 &lt; folhas 50 &lt; bloqueio 60); **formatos** (hora relativa no feed, absoluta e destacada em prazos); **movimento** (140/240 ms, `prefers-reduced-motion` zera tudo). **Segue aberto:** breakpoints e larguras além da moldura de 400 px (coluna de leitura do modo jornal); extração dos valores para tokens nomeados. ~~Tabela única das strings canônicas de honestidade~~ — **resolvida**: é a [§11.1](#111-tabela-única-de-honestidade-canônica-conceitual), rascunhada nos ciclos do protótipo e agora canônica.
