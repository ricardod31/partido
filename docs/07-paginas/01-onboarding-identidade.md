...
flowchart TD
    ...
    GEN --> BKP["P-ON-04 Backup da frase mnemônica"]
    BKP --> PASS["P-ON-05 Definir passphrase"]
    PASS --> NET["P-ON-06 Rede/anonimato (Tor)"]
    NET --> PEND["P-ON-07 Pendente de admissão"]
    PEND -->|"célula admite (Commit Add)"| HOME["Panorama P-NAV-01"]
    RESTORE --> UNLOCK["P-ON-08 Desbloqueio"]
    UNLOCK --> HOME
```

Princípio de fluxo: **a identidade é criada e resguardada *antes* de tocar o servidor** (a seed nasce no cliente, §3 do doc 03). O registro (envio de chaves públicas + convite) só ocorre ao final, e a **admissão** (virar membro de fato) é uma operação da célula, não do cadastro.
...
3. **Funcionalidades.** Três ações: *criar identidade* (tenho um convite), *restaurar* (já tenho frase mnemônica), *ler o jornal público* (sem conta). Link discreto para "o que é isto" (visão) e para o painel de Exposição (P-SEG-06).
...
5. **Experiência e layout.** `Ceremony` de um passo. TrustChip "Anônimo" explicando que o convite não revela seu padrinho ao servidor. Erro de convite inválido/expirado é claro e não culpabiliza.
...
5. **Experiência e layout.** `HonestyCallout` de opsec (doc 06 §8.1): *"Escolha um pseudônimo sem relação com seu nome, trabalho ou redes."* E o **limite honesto**: *"Um mesmo aparelho pode ligar seus pseudônimos por rede/horário — pseudônimo não é anonimato de rede; para isso, use Tor (próximo passo)."* Prévia do identicon derivado do handle.
6. **Estados.** Pseudônimo já em uso (checagem só na etapa de registro, para não vazar enumeração antes da hora); geração em curso (brevíssima).
7. **Restrições.** P3; UX2 (o `user_id` é âncora estável, mas os **handles** exibidos são por-organismo); doc 06 §8.1 (não insinuar não-vinculabilidade que a rede desfaz).
...
2. **Quem chega.** Identidade recém-gerada.
3. **Funcionalidades.** Exibir a frase mnemônica (lista de palavras estilo BIP-39 que codifica a seed, doc 03 §3.2); confirmação por reintrodução de palavras sorteadas; guia de backup seguro.
4. **Dinâmica.** `Ceremony` full-screen, focada, com captura de tela desencorajada. Confirmação obrigatória antes de prosseguir.
...
2. **Quem chega.** Backup confirmado.
...
3. **Funcionalidades.** Ativar Tor/onion service (doc 03 §9); configurar **fail-closed** (padrão ligado); *bridges*/pluggable transports para redes que bloqueiam Tor (doc 06 §8.2).
4. **Dinâmica.** O cliente prioriza o onion service da organização; testa a conexão anônima. **Fail-closed é o padrão** e a UI explica o que ele fará: recusar operações sensíveis sem canal anônimo, em vez de cair para clearnet.
...
5. **Experiência e layout.** EmptyState explicativo, sem falsa barra de progresso. Deixa claro que a espera é **social** (alguém da célula precisa agir), não técnica.
...
6. **Estados.** Passphrase errada (rate-limit local via keystore); servidor não confere o *pinning* (alerta forte de possível *relay*/servidor trocado — doc 03 §5.2); offline (desbloqueio local funciona; login espera rede).
...
1. **Objetivo.** Recriar a identidade num aparelho novo a partir da frase.
2. **Quem chega.** Quem trocou/perdeu o aparelho e tem a frase.
...
5. **Experiência e layout.** `Ceremony` com QR de vida curta; `HonestyCallout`: *"Quem vê este QR ganha sua identidade — faça isto a sós, offline."* Desencoraja captura de tela.
6. **Estados.** QR expirado; leitura confirmada; ambiente inseguro alertado.
...
- **Convite cegado** (P-ON-02): esquema concreto — impacta o metadado de recrutamento (A3).
- **Recuperação social** (P-ON-11): protocolo de re-atestação — bloqueia a tela final.
- **Calibração Argon2id + keystore** (P-ON-05): parâmetros por classe de dispositivo.
