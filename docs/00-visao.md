...
- **P2 — O servidor é cego.** Todo conteúdo é criptografado no cliente, de ponta a ponta. O servidor armazena e roteia envelopes cifrados que ele **rejeita** quando malformados e **não consegue ler** quando válidos. O servidor não guarda nenhum dado de identificação pessoal (PII) por construção: não existe campo para nome real, e-mail ou telefone.
...
- **P4 — Centralismo democrático como fluxo de dados.** O método decisório da tradição leninista vira arquitetura de informação: **eleição e informes sobem** (célula elege delegados e reporta), **decisões vinculantes descem** (resoluções de instâncias superiores são publicadas para as inferiores), **discussão precede o voto** (toda deliberação tem fase de discussão antes da votação), **prestação de contas é periódica** (mandatos expiram e são revogáveis).
